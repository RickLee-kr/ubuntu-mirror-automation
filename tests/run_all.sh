...
FAIL=0
for t in test_install.sh test_validate.sh test_validate_fixture.sh test_nginx.sh test_systemd.sh test_simplified_install.sh test_dashboard.sh test_offline_mirror.sh test_upgrade_profile.py test_selective_mirror.py test_selective_orchestration_lock.sh test_selective_runtime_migration.py test_sync_by_hash.py test_security_compat.py test_release_upgraders.py test_legacy_releases.py test_analyze_upgrade_discovery.py test_collect_dp_upgrade_readiness.sh test_dp_upgrade_preflight.sh test_dp_os_upgrade.sh test_discover_upgrade_requirements.sh test_destructive_confirmation.sh test_phase1_finalize.sh test_ntp_pre_transition_quiesce.sh test_dp_offline_upgrade_xenial_to_bionic.sh test_dp_offline_upgrade_bionic_to_focal.sh test_dp_offline_upgrade_focal_to_jammy.sh test_dp_offline_upgrade_jammy_to_noble.sh test_dp_phase2_bundle.sh test_dp_phase2_client_stage.sh test_client_manifest_signing.sh test_distupgrade_config_ascii.sh test_distupgrade_source_compat.py test_prepare_backup_staging.sh test_migrate_apt_mirror_to_root.sh; do
  echo "======== Running $t ========"
  if [[ "$t" == *.py ]]; then
    if python3 "$t"; then
      echo "OK $t"

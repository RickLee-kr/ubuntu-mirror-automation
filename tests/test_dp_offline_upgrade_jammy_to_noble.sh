...
SCRIPT_IN="${ROOT}/client/dp-offline-upgrade-jammy-to-noble.sh.in"
...
EOF
  set +e
  DP_OFFLINE_TEST_ROOT="$fake" bash "$BUILT" >"$fake/out-noble.txt" 2>&1
  rc=$?
  set -e
...
make_runner_fixture() {
  ...
  printf 'deb [arch=amd64] http://221.139.249.111/hops/jammy-to-noble/ubuntu jammy main universe\n' \
  ...
  {
    echo 'deb [arch=amd64] http://221.139.249.111/hops/jammy-to-noble/ubuntu noble main universe'
    echo 'deb [arch=amd64] http://221.139.249.111/hops/jammy-to-noble/ubuntu noble-updates main universe'
    echo 'deb [arch=amd64] http://221.139.249.111/hops/jammy-to-noble/ubuntu noble-security main universe'
    echo 'deb [arch=amd64] http://221.139.249.111/hops/jammy-to-noble/ubuntu noble-backports main universe'
...
LOCAL_URI='http://221.139.249.111/hops/jammy-to-noble/ubuntu'
...
open(bin_path + '.env', 'w').write(
    'STELLAR_EXPECTED_MIRROR_URI=http://221.139.249.111/hops/jammy-to-noble/ubuntu\n'
    'STELLAR_TARGET_CODENAME=noble\n'
    'STELLAR_SOURCE_CODENAME=jammy\n'
    'STELLAR_COMPONENTS=main universe\n'
...
EOF
  ...
  set +e
  DP_OFFLINE_TEST_ROOT="$hop_k" bash "$B2F_STUB" --preflight-only >"$hop_k/out.txt" 2>&1
  rc=$?
  set -e
...
# Scope to the guard function only (LXD helpers follow before run_os_preflight).
av_body="$(awk '/^check_cross_release_candidates\(\)/,/^# --- Offline LXD inventory|^lxd_pkg_installed_p\(\)|^run_os_preflight\(\)/' "$SCRIPT_IN")"
...
LXD_INVENTORY_TIMEOUT_SECS="${DP_OFFLINE_LXD_INVENTORY_TIMEOUT_SECS:-20}"
LXD_PREFLIGHT_CLASS=""
...
EOS
  # Extract LXD helpers from template (through emit_lxd_snap_failure_fields).
  awk '
    /^# --- Offline LXD inventory/ {p=1}
    p {
      print
      if ($0 ~ /^emit_lxd_snap_failure_fields\(\)/) emit=1
      if (emit && $0 ~ /^}$/) exit
    }
  ' "$SCRIPT_IN"
} >"$LXD_HARNESS"
...
if [[ -f "${HANDOFF_HARNESS:-}" ]]; then
  ...
  export DP_OFFLINE_TEST_ROOT="$bv" DP_OFFLINE_TEST_HANDOFF=1 TEST_ROOT="$bv"
...
if [[ -f "${HANDOFF_HARNESS:-}" ]]; then
  ...
  export DP_OFFLINE_TEST_ROOT="$by" DP_OFFLINE_TEST_HANDOFF=1 TEST_ROOT="$by"
...
# Static contract: no --format on storage inventory; errors.txt guarded
if grep -nE 'lxc storage volume list --format|lxc storage list --format' "$SCRIPT_IN"; then
  fail "18.CL storage inventory must not use --format"
else
  pass "18.CL storage inventory avoids --format csv"
fi
if grep -q 'if \[\[ -f "\${evid}/errors.txt" \]\]' "$SCRIPT_IN" \
  || grep -q 'if [[ -f "${evid}/errors.txt" ]]' "$SCRIPT_IN"; then
  pass "18.CL-b errors.txt absence guarded with if"
else
  # Accept explicit || true form as well
  if grep -n 'errors.txt' "$SCRIPT_IN" | grep -q '|| true'; then
    pass "18.CL-b errors.txt absence guarded"
  else
    fail "18.CL-b errors.txt pipefail guard missing"
  fi
fi
...
grep -q 'CRITICAL_OS_HOLD_RESTORE=SKIPPED_PACKAGE_TRANSITION_EVIDENCE' "$SCRIPT_IN" \
  && pass "28.static hold-restore evidence skip separated" \
  || fail "28.static hold-restore evidence skip missing"
...
printf '\n' | env DP_OFFLINE_TEST_ROOT="$hop_ops" DP_OFFLINE_FAKE_MIRROR_TRUST=1 \
  ...
  bash "$STUB" >"$hop_ops/out-reject.txt" 2>&1
rc=$?
set -e
if [[ "$rc" -eq 21 ]] \
...
env DP_OFFLINE_TEST_ROOT="$hop_ops" DP_OFFLINE_FAKE_MIRROR_TRUST=1 \
  DP_OFFLINE_FAKE_DP_VERSION=6.2.0 DP_OFFLINE_FAKE_ROLE=AIO \
  DP_OFFLINE_FAKE_CONFIRM=UPGRADE-JAMMY-TO-NOBLE \
  bash "$STUB" >"$hop_ops/out-commit.txt" 2>&1
rc=$?
set -e
if [[ "$rc" -eq 0 ]] \
...
LOCAL_URI_J2N='http://221.139.249.111/hops/jammy-to-noble/ubuntu'
...
# Phase 2 auto-run must be absent
if grep -nE 'bringup_py3_dp_after_os_upgrade|auto.*Phase 2|Phase 2.*auto' "$SCRIPT_IN" \
  | grep -viE 'NOT|Does NOT|never|OUT_OF_SCOPE|separate|forbidden|must not'; then

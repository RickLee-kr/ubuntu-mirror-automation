...
bash -n "${ROOT}/client/stage-dp-phase2-6.5.0.sh" && pass "compat helper bash -n" || fail "compat helper bash -n"
...
for start_mode in 600 644; do
  ...
  before_inode="$(stat -c '%i %s' "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar")"
  out="$(run_publisher "$fx" 6.5.0)"
  echo "$out" | grep -q 'RELEASE_ENV_MODE=644' && pass "mode start=${start_mode} -> 644 reported" \
    || fail "mode start=${start_mode} report"
  [[ "$(stat -c '%a' "${fx}/6.5.0/current/release.env")" == "644" ]] \
  ...
  after_inode="$(stat -c '%i %s' "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar")"
...
(
  umask 077
  run_publisher "$fx" 6.5.0 >/dev/null
)
[[ "$(stat -c '%a' "${fx}/6.5.0/current/release.env")" == "644" ]] \
...
printf 'IMAGE_LIST_COUNT=156\n' >>"${fx}/6.5.0/current/release.env"
run_publisher "$fx" 6.5.0 >/dev/null
envf="${fx}/6.5.0/current/release.env"
grep -qE '^TARGET_DP_VERSION=6.5.0$' "$envf" && pass "TARGET_DP_VERSION first-class" || fail "TARGET_DP_VERSION"
grep -qE '^PHASE2_ARTIFACT_VERSION=6.5.0$' "$envf" && pass "PHASE2_ARTIFACT_VERSION" || fail "PHASE2"
grep -qE '^DP_PHASE2_VERSION=6.5.0$' "$envf" && pass "deprecated DP_PHASE2_VERSION preserved" || fail "DP_PHASE2_VERSION"
grep -qE '^IMAGE_LIST_COUNT=156$' "$envf" && pass "existing key preserved" || fail "key preserve"
grep -qE '^STABLE_BUNDLE_NAME=dp_bundle_6.5.0-current.tar$' "$envf" && pass "STABLE_BUNDLE_NAME" || fail "stable"
...
fx="${WORKDIR}/badver"
make_fixture "$fx" 600
before="$(sha256sum "${fx}/6.5.0/current/release.env" | awk '{print $1}')"
set +e
out="$(run_publisher "$fx" 'not.a.version' 2>&1)"
rc=$?
set -e
[[ "$rc" -ne 0 ]] && pass "malformed target version STOP" || fail "malformed should STOP"
after="$(sha256sum "${fx}/6.5.0/current/release.env" | awk '{print $1}')"
...
do
  ...
  make_fixture "$fx" 600
  printf '%s\n' "$secret_line" >>"${fx}/6.5.0/current/release.env"
  before="$(sha256sum "${fx}/6.5.0/current/release.env" | awk '{print $1}')"
  set +e
  out="$(run_publisher "$fx" 6.5.0 2>&1)"
  rc=$?
  set -e
  ...
  # no temp leftovers
  if compgen -G "${fx}/6.5.0/current/.release.env.*" >/dev/null 2>&1; then
...
rel="${fx}/6.5.0/releases/20260726T155911Z"
mkdir -p "$rel"
mv "${fx}/6.5.0/current/"* "$rel/"
rmdir "${fx}/6.5.0/current"
ln -sfn "$rel" "${fx}/6.5.0/current"
ready_before="$(sha256sum "${fx}/ready-state/READY" | awk '{print $1}')"
bundle_before="$(stat -c '%i %s' "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar")"
current_before="$(readlink -f "${fx}/6.5.0/current")"
bundle_sha_before="$(sha256sum "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar" | awk '{print $1}')"
run_publisher "$fx" 6.5.0 >/dev/null
ready_after="$(sha256sum "${fx}/ready-state/READY" | awk '{print $1}')"
bundle_after="$(stat -c '%i %s' "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar")"
current_after="$(readlink -f "${fx}/6.5.0/current")"
bundle_sha_after="$(sha256sum "${fx}/6.5.0/current/dp_bundle_6.5.0-current.tar" | awk '{print $1}')"
...
before="$(sha256sum "${fx}/6.5.0/current/release.env" | awk '{print $1}')"
set +e
out="$(DP_PHASE2_RELEASE_ENV_ATOMIC_FAIL_REPLACE=1 run_publisher "$fx" 6.5.0 2>&1)"
rc=$?
set -e
...
make_fixture "$fx" 600
run_publisher "$fx" 6.5.0 >/dev/null
# Simulate nginx alias: /dp-phase2/6.5.0/ -> current/
HTTP_ROOT="${WORKDIR}/http_root"
mkdir -p "${HTTP_ROOT}/dp-phase2/6.5.0"
cp -a "${fx}/6.5.0/current/release.env" "${HTTP_ROOT}/dp-phase2/6.5.0/release.env"
# Trap: wrong canonical would look under current/current/
mkdir -p "${HTTP_ROOT}/dp-phase2/6.5.0/current"
printf 'WRONG_NESTED\n' >"${HTTP_ROOT}/dp-phase2/6.5.0/current/release.env"
...
code="$(curl -sS -o "${WORKDIR}/http-body" -w '%{http_code}' --max-time 10 \
  'http://127.0.0.1:8767/dp-phase2/6.5.0/release.env')"
[[ "$code" == "200" ]] && pass "canonical HTTP 200" || fail "canonical HTTP code=${code}"
fs_sha="$(sha256sum "${fx}/6.5.0/current/release.env" | awk '{print $1}')"
...
fx="${WORKDIR}/perms"
make_fixture "$fx" 600
uid_before="$(stat -c '%u' "${fx}/6.5.0/current/release.env")"
gid_before="$(stat -c '%g' "${fx}/6.5.0/current/release.env")"
run_publisher "$fx" 6.5.0 >/dev/null
uid_after="$(stat -c '%u' "${fx}/6.5.0/current/release.env")"
gid_after="$(stat -c '%g' "${fx}/6.5.0/current/release.env")"
mode_after="$(stat -c '%a' "${fx}/6.5.0/current/release.env")"
...
rm -f "${HTTP_ROOT}/dp-phase2/6.5.0/release.env"
cp -a "${fx}/6.5.0/current/release.env" "${HTTP_ROOT}/dp-phase2/6.5.0/release.env"
set +e
out="$(

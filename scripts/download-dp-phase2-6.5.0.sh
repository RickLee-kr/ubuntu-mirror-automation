#!/usr/bin/env bash
# Download, verify, bundle, and atomically publish DP Phase 2 6.5.0 artifacts from ACPS.
...
verify_release_dir() {
  ...
  dp2_check_image_list "${files_dir}/images-6.5.0.list" >/dev/null
  local stable
...
cmd_sync() {
  ...
  local list_count
  list_count="$(dp2_check_image_list "${STAGING_DIR}/files/images-6.5.0.list" | tail -n1)"

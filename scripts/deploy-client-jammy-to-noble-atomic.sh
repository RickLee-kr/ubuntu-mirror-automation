#!/usr/bin/env bash
# Atomic deploy of jammy→noble approved client artifacts:
...
ARTIFACT="${ROOT}/artifacts/client/dp-offline-upgrade-jammy-to-noble.sh"
SHAFILE="${ARTIFACT}.sha256"
HOP_DIR="${ROOT}/artifacts/client/jammy-to-noble"
HOP_NAME="jammy-to-noble"
DEST_ROOT="${DEST_ROOT:-/var/spool/apt-mirror/client}"
NAME="dp-offline-upgrade-jammy-to-noble.sh"
...
spec = importlib.util.spec_from_file_location("build_client_j2n", build_py)

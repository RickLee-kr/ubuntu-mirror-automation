#!/usr/bin/env bash
# Atomic deploy of focal→jammy approved client artifacts:
...
ARTIFACT="${ROOT}/artifacts/client/dp-offline-upgrade-focal-to-jammy.sh"
SHAFILE="${ARTIFACT}.sha256"
HOP_DIR="${ROOT}/artifacts/client/focal-to-jammy"
HOP_NAME="focal-to-jammy"
DEST_ROOT="${DEST_ROOT:-/var/spool/apt-mirror/client}"
NAME="dp-offline-upgrade-focal-to-jammy.sh"
...
EXPECTED_ARTIFACT_SHA="${EXPECTED_ARTIFACT_SHA:-5bcf2dbb1068030aa3e4289600c85adcfdeb02b8e274780f34f479c3637bf0f5}"
...
spec = importlib.util.spec_from_file_location("build_client_f2j", build_py)
